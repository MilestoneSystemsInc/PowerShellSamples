...
Register-ScheduledJob @jobParams -ScriptBlock {
    ...
    try {
        ...
        Start-Transcript -Path $WorkingDirectory\ScheduledCameraReport.log
        $reportsFolder = New-Item -Path (Join-Path $WorkingDirectory 'reports\') -ItemType Directory -Force | Select-Object -ExpandProperty FullName
        Write-Host 'Cleaning up camera reports older than 30 days'
        Get-ChildItem -Path $reportsFolder\Camera-Report_*.csv | Where-Object CreationTime -lt (Get-Date).AddDays(-30) | Remove-Item -Verbose
        ...
        $csvPath = Join-Path $reportsFolder "CameraReport_$(Get-Date -Format FileDateTime).csv"
        Write-Host "Running Get-CameraReport and saving results to $csvPath"
        Get-CameraReport -Verbose | Export-Csv -Path $csvPath -NoTypeInformation -Force
    }
    catch {
        Write-Error $_
    }
